...
- **Backend** — the content-agnostic index beneath each project. It keeps a
  ...
  (`coder/hnsw`, approximate nearest-neighbour search) — and persists within a
  narrowed `fs.FS`.
- **Embeddings Engine** — a single `Embeddings` interface with two
  ...
  serves many projects across many workspaces.
...
| [`vek`][vek] | Pure-Go SIMD vector maths | [MIT][vek-licence] |
