...
GOLANGCI_LINT_VERSION ?= $(shell $(TOOLSDIR)/get_version.sh 1.26 v2.11.4)
REVIVE_VERSION ?= $(shell $(TOOLSDIR)/get_version.sh 1.26 v1.15.0)
...
FIND_FILES_PRUNE_RULES ?= -name vendor -o -name .git -o -name node_modules \
			  -o -name .tmp -o -name .srclight -o -name .srcdex \
			  -o -name .claude
