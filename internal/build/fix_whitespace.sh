...
#
# Automatically prunes .git, .tmp, node_modules, .srclight, .srcdex and
# .claude directories
#
...
run_find() {
	...
	set -- \( -type d \( -name .git -o -name .tmp -o -name node_modules -o -name .srclight -o -name .srcdex -o -name .claude \) \) -prune -o "$@" -type f
